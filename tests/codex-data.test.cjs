...
test('collectCodexData aggregates real token deltas without double counting snapshots', async () => {
  ...
  const records = [
    { timestamp: '2026-07-17T01:00:00Z', payload: { type: 'token_count', info: { last_token_usage: { input_tokens: 80, output_tokens: 20, total_tokens: 100 }, total_token_usage: { input_tokens: 80, output_tokens: 20, total_tokens: 100 } } } },
    { timestamp: '2026-07-17T01:01:00Z', payload: { type: 'token_count', info: { last_token_usage: { input_tokens: 40, output_tokens: 10, total_tokens: 50 }, total_token_usage: { input_tokens: 120, output_tokens: 30, total_tokens: 150 } }, rate_limits: { primary: { used_percent: 25, window_minutes: 300, resets_at: 1784250000 }, secondary: { used_percent: 40, window_minutes: 10080, resets_at: 1784850000 } } } },
  ];
...
test('collectCodexData sums per-event last usage instead of repeating session snapshots', async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), 'codex-widget-last-usage-'));
  const day = path.join(home, 'sessions', '2026', '07', '18');
  await fs.mkdir(day, { recursive: true });
  const records = [
    { timestamp: '2026-07-18T02:00:00Z', payload: { type: 'token_count', info: { last_token_usage: { input_tokens: 8, output_tokens: 2, total_tokens: 10 }, total_token_usage: { input_tokens: 800, output_tokens: 200, total_tokens: 1000 } } } },
    { timestamp: '2026-07-18T02:01:00Z', payload: { type: 'token_count', info: { last_token_usage: { input_tokens: 16, output_tokens: 4, total_tokens: 20 }, total_token_usage: { input_tokens: 816, output_tokens: 204, total_tokens: 1020 } } } },
  ];
  await fs.writeFile(path.join(day, 'rollout.jsonl'), records.map(JSON.stringify).join('\n'));
