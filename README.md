...
- 今日 Token：读取 `%USERPROFILE%\.codex\sessions` 中本机当天每次 `token_count` 的 `last_token_usage` 增量；不包含其他设备或云端的当天活动。
...
今日 Token 的逐事件统计方式参考了 MIT 开源项目 [1nuYasha-cck/codex-quota-widget](https://github.com/1nuYasha-cck/codex-quota-widget)，并针对本项目的数据合并逻辑重新实现。
