...
一个「Codex 额度面板」应用。目前暂时只做了 Windows 版。面板同步显示来自 Codex 官方的：每周使用限额、当前可用额度重置、今日 Token 用量和累计 Token 用量。

> 本项目不是 OpenAI 官方产品。它通过用户电脑上已登录的 OpenAI Codex 官方 app-server 读取账户数据，并读取本机 Codex 会话日志补齐官方尚未生成的当天实时用量；不依赖其他额度插件，也不上传任何数据。

## 功能

- 与设计稿一致的无边框轻拟态面板，灰色轻拟态风格
- 语言切换、最小化、关闭三个等尺寸窗口按钮；关闭后驻留系统托盘，最小化后进入任务栏
- 中文 / 英文切换，首次使用自动跟随 Windows 系统语言
- 面板高度固定；重置卡片大于 3 个时，显示独立滚动条，鼠标悬停面板滚轮也可滚动重置卡片列表
